...
class ManagementForm extends Component {
  validateSchema = values => {
    ...
    let outputSchema = {
      name: [[value => !isEmpty(value), 'Tên quyền không được để trống!']]
    };
            ...
            <Form onSubmit={handleSubmit}>
              <Row>
                <Col sm="6 pd-right-10">
                  <FormGroup>
                    <Label className="user-form__label">Tên quyền</Label>
...
class ManagementModal extends Component {
  ...
  render() {
    ...
    return (
      <Mutation
        mutation={mode === 'create' ? mutation.createRole : mutation.updateRole}
        onCompleted={data => {
          mode === 'create'
            ? notify.success('Tạo quyền thành công!')
            : notify.success('Chỉnh sửa quyền thành công!');
          closeModal();
          refresh();
        }}
      >
        {(saveRole, { loading, error, data }) => {
          if (loading) {
