export default {
  ...
  hasLowerChar(value) {
    const HAS_LOWER_CHAR = /^(?=.*[a-z])/; // eslint-disable-line no-useless-escape
    return HAS_LOWER_CHAR.test(value);
  },
  hasUpperChar(value) {
    const HAS_UPPER_CHAR = /^(?=.*[A-Z])/; // eslint-disable-line no-useless-escape
    return HAS_UPPER_CHAR.test(value);
  },
  hasNumber(value) {
    const HAS_NUMBER = /^(?=.*[0-9])/; // eslint-disable-line no-useless-escape
